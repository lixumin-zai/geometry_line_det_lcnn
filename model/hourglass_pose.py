...
from model.multitask_learner import MultitaskHead
...
class HourglassNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer1(x)  # [1, 128, 128, 128]
        

        x = self.maxpool(x) # [n, 128, 64, 64]
        x = self.layer2(x)  # [n, 256, 64, 64]
  
        x = self.layer3(x)  # [n, 256, 64, 64]
...
if __name__ == "__main__":
    ...
    # 创建测试数据
    data = np.random.randint(0, 256, (1, 3, 512, 512)).astype(np.float32)
    ...
    y_preds = model(data)
    print(y_preds[0][0].shape)
    # print(y_preds[1].shape)
    # 遍历输出 -- 可能多个输出(4)，也可能只含有一个输出[x]
    # for i in y_preds[0]:
    #     print(i.shape)
