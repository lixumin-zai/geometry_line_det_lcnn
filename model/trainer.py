...
from model.utils import recursive_to
...
class Trainer(object):
    ...
    def validate(self):
        ...
        viz = os.path.join(self.out, "viz", f"{self.iteration * config['batch_size_eval']:09d}")
        npz = os.path.join(self.out, "npz", f"{self.iteration * config['batch_size_eval']:09d}")

        os.path.exists(viz) or os.mkdir(viz)
        os.path.exists(npz) or os.mkdir(npz)
        ...
        with torch.no_grad():
            for batch_idx, (image, meta, target) in enumerate(self.val_loader):
                ...
                for i in range(H["jmap"].shape[0]):
                    index = batch_idx * config['batch_size_eval'] + i
                    np.savez(
                        f"{npz}/{index:06}.npz",
        ...
        torch.save(
            ...
            os.path.join(self.out, "checkpoint_latest.pth"),
        )
        shutil.copy(
            os.path.join(self.out, "checkpoint_latest.pth"),
            os.path.join(npz, "checkpoint.pth"),
        )
        if self.mean_loss < self.best_mean_loss:
            self.best_mean_loss = self.mean_loss
            shutil.copy(
                os.path.join(self.out, "checkpoint_latest.pth"),
                os.path.join(self.out, "checkpoint_best.pth"),
            )
    ...
    def train_epoch(self, test=False):
        ...
        for batch_idx, (image, meta, target) in enumerate(self.train_loader):
            ...
            if self.iteration % 4 == 0:
                ...
                text = f"{self.epoch:03}/{self.iteration * self.batch_size:09}" "| ".join(map("{:.5f}".format, self.avg_metrics[0])) \
    ...
    def _write_metrics(self, size, total_loss, prefix, do_print=False):
        for i, metrics in enumerate(self.metrics):
            if i == 0 and do_print:
                text = f"{self.epoch:03}/{self.iteration * self.batch_size:08}" + " | ".join(map("{:.8f}".format, metrics / size))
                print("\r", end="")
                print(text)
                with open(f"{self.out}/loss.txt", "a") as f:
                    f.write(text+"\n")
